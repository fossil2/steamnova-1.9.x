...
<div style="background:url('{$dpath}images/buildings.webp') no-repeat center center; -webkit-background-size: cover;
  -moz-background-size: cover; -o-background-size: cover;background-size: cover;"
   class="itemShow d-flex justify-content-center align-items-start w-100 position-relative border-gray">
...
<div class="d-flex flex-column w-100 bg-light-black itemInfo overflow-auto scroll">
    ...
    <button class="btn btn-dark py-0 px-10 border border-secondary fs-12 text-yellow" {if $ID == 43}<a href="#" onclick="return Dialog.info({$ID})">{$LNG.bd_jump_gate_action}</a>{/if}</button>
  </div>
</div>
{/foreach}
</div>
...
{if !empty($Queue)}
 <div id="buildlist" class="ItemsWrapper d-flex flex-wrap justify-content-start w-100 mx-auto my-2 py-2 bg-black border-gray">
