...
$game_speed = mysqli_fetch_all(mysqli_query($connection, "SELECT FLOOR(game_speed/2500) FROM config")) [0][0];

$bots = mysqli_fetch_all(mysqli_query($connection, "SELECT id FROM users WHERE email='bot'"));
...
for ($j = 0;$j <= count($bots) - 1;$j++)
{
	...
	// Calculate building costs
        ...
        $data_building = mysqli_fetch_array(mysqli_query($connection, "SELECT cost901, cost902, cost903, factor, elementID FROM vars WHERE name='$name'"));
	    ...
	    // Building time!
            ...
            mysqli_query($connection, "UPDATE planets SET b_building = $endTime WHERE id=$id_planet");
            mysqli_query($connection, "UPDATE planets SET b_building_id = '$b_building_id' WHERE id=$id_planet");
            mysqli_query($connection, "UPDATE planets SET metal = metal - $metal WHERE id=$id_planet");
            mysqli_query($connection, "UPDATE planets SET crystal = crystal - $crystal WHERE id=$id_planet");
            mysqli_query($connection, "UPDATE planets SET deuterium = deuterium - $deuterium WHERE id=$id_planet");
        }
        else
        {
            $time = time();
            ...
            mysqli_query($connection, "UPDATE users SET b_tech_planet = $id_planet WHERE id=$id_bot");
            mysqli_query($connection, "UPDATE users SET b_tech = $endTime WHERE id=$id_bot");
            mysqli_query($connection, "UPDATE users SET b_tech_id = $id_vars WHERE id=$id_bot");
            mysqli_query($connection, "UPDATE users SET b_tech_queue = '$b_tech_queue' WHERE id=$id_bot");
            mysqli_query($connection, "UPDATE planets SET metal = metal - $metal WHERE id=$id_planet");
            mysqli_query($connection, "UPDATE planets SET crystal = crystal - $crystal WHERE id=$id_planet");
            mysqli_query($connection, "UPDATE planets SET deuterium = deuterium - $deuterium WHERE id=$id_planet");
        }
	...
	// Forcing bot to attack in order to finish construction (BitNova finishes construction when there is a battle or when the user logs in to the game)
        ...
        mysqli_query($connection, "UPDATE users SET onlinetime = $start_time WHERE id=$id_bot");
