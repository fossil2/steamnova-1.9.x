...
function attack($id_owner, $fleet_start_id, $fleet_start_galaxy, $fleet_start_system, $fleet_start_planet) {
	...
	mysqli_query($connection, "UPDATE planets SET spy_sonde = 0 WHERE id=$fleet_start_id");
...
$get_all_bots = mysqli_fetch_all(mysqli_query($connection, "SELECT id FROM users WHERE email = 'bot'"));

for($i=0; $i<=count($get_all_bots)-1; $i++) {
	$id_bot = $get_all_bots[$i][0];
	$get_planets = mysqli_fetch_all(mysqli_query($connection, "SELECT id, galaxy, system, planet, metal, crystal, deuterium FROM planets WHERE id_owner = $id_bot"));
