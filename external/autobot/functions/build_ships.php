...
for ($j = 0;$j <= count($bots) - 1;$j++)
{
	...
	// Get user planet
        ...
        $bot_resources = mysqli_fetch_array(mysqli_query($connection, "SELECT metal, crystal, deuterium FROM planets WHERE (id=$id_planet)"));
	$metal = $bot_resources[0];
	$crystal = $bot_resources[1];
	$deuterium = $bot_resources[2];
        mysqli_query($connection, "UPDATE planets SET spy_sonde = 0 WHERE id=$id_planet");
	...
	// Check if something can be built by a bot
		...
		if ( ($id_bot % 2 != 0) && ($metal >= 60000 ) && ($crystal >= 50000 ) && ($deuterium >= 15000 ) )
			{
				mysqli_query($connection, "UPDATE planets SET metal = metal - 60000 WHERE id = $id_planet");
				mysqli_query($connection, "UPDATE planets SET crystal = crystal - 50000 WHERE id = $id_planet");
				mysqli_query($connection, "UPDATE planets SET deuterium = deuterium - 15000 WHERE id = $id_planet");
				mysqli_query($connection, "UPDATE planets SET destructor = destructor + 1 WHERE id = $id_planet");
			}
		else if ( ($id_bot % 2 == 0) && ($metal >= 5000000 ) && ($crystal >= 4000000 ) && ($deuterium >= 1000000 ) )
			{
				mysqli_query($connection, "UPDATE planets SET metal = metal - 5000000 WHERE id = $id_planet");
				mysqli_query($connection, "UPDATE planets SET crystal = crystal - 4000000 WHERE id = $id_planet");
				mysqli_query($connection, "UPDATE planets SET deuterium = deuterium - 1000000 WHERE id = $id_planet");
				mysqli_query($connection, "UPDATE planets SET dearth_star = dearth_star + 1 WHERE id = $id_planet");
		}
		else {
