...
class ShowInformationPage extends AbstractGamePage
{
	...
	public function sendFleet()
	{
		...
		if ($TargetGate === false || $TargetPlanet == $PLANET['id'])
		{
			$this->sendJSON(array(
		...
		foreach($reslist['fleet'] as $Ship)
		{
			...
			// Clean ship count by removing thousands separators and converting to integer
			$shipCount = is_string($Ships[$Ship]) ? str_replace(',', '', $Ships[$Ship]) : $Ships[$Ship];
			$shipCount = (int) $shipCount;
			
			$ShipArray[$Ship]	= max(0, min($shipCount, $PLANET[$resource[$Ship]]));
