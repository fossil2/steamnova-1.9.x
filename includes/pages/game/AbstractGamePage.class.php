...
abstract class AbstractGamePage
{
    ...
    protected function GetFleets() {
        ...
        if (isset($USER['id']) && isset($PLANET['id'])) {
            $fleetTableObj->setUser($USER['id']);
            $fleetTableObj->setPlanet($PLANET['id']);
            return $fleetTableObj->renderTable();
        }
    ...
    /**
     * Load user planets from database
     */
    protected function loadUserPlanets()
    {
        global $USER;
        ...
        $sql = "SELECT * FROM %%PLANETS%% 
                ...
                AND destruyed = 0 
                ORDER BY planet_type, galaxy, system, planet";
        
        $USER['PLANETS'] = $db->select($sql, array(
            ':userId' => $USER['id']
        ));
        
        if (empty($USER['PLANETS'])) {
            error_log("No planets found for user ID: " . $USER['id']);
            $USER['PLANETS'] = array();
        }
    }
