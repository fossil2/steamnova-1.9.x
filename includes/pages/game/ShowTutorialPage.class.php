...
//Alli-Mitglied
	...
	$db = Database::get();
 	...
 	$query = $db->selectsingle("SELECT * FROM buddy WHERE `sender` = '". $USER['id'] ."';");
	if($query>1){
...
$query = $db->selectsingle("SELECT count(*) AS planet_count FROM planets WHERE `id_owner` = '". $USER['id'] ."';");
