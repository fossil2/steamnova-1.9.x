...
class MissionCaseSpy extends MissionFunctions implements Mission
{
	...
	function TargetEvent()
	{
		...
		// Zeigt die erforderliche Anzahl an Frachtschiffen an, die zum Plündern von Ressourcen erforderlich sind
		$needed_kt = 0 ;
		$needed_gt = 0 ;
		$total = 0 ;
		if(!empty($spyData[900])){
			foreach ( $spyData [ 900 ] as $elementID => $amount ) {
				$total += Round($amount/2);
			}
			$ship_kt			  = $pricelist[202];
			$ship_gt			  = $pricelist[203];
			$ship_storage_kt	  = $ship_kt['capacity'];
			$ship_storage_gt	  = $ship_gt['capacity'];
			$needed_kt					 = Round(ceil($total / $ship_storage_kt));
			$needed_gt					 = Round(ceil($total / $ship_storage_gt));
		} 

		// I'm use template class here, because i want to exclude HTML in PHP.
		// Ich verwende hier die Template-Klasse, weil ich HTML in PHP ausschließen möchte.
