...
class GalaxyRows
{
	...
	protected function getAllowedMissions()
	{
		...
		$this->galaxyData[$this->galaxyRow['planet']]['missions']	= array(
			...
			18	=> $this->galaxyRow['id_owner']== NULL,
		);
	}
	...
	protected function getPlayerData()
	{
		...
		$Class		 		= userStatus($this->galaxyRow, $IsNoobProtec);
				...
				//test  'NULL' seit PHP 7.2.0 als DEPRECATED (veraltet) markiert und wurde in PHP 8.0.0 ENTFERNT.
				$d = $this->galaxyRow['username'];
				if ( is_null ( $d ) )
				{
					//echo '$d ist Null' . "\n";
					$d = '';
        ...
        $this->galaxyData[$this->galaxyRow['planet']]['user']	= array(
			...
			'username'		=> htmlspecialchars($d, ENT_QUOTES, "UTF-8"),
			...
			'playerrank'	=> isModuleAvailable(25)?sprintf($LNG['gl_in_the_rank'], htmlspecialchars($d,ENT_QUOTES,"UTF-8"), $this->galaxyRow['total_rank']):htmlspecialchars($d,ENT_QUOTES,"UTF-8"),
			'class'			=> $Class,
