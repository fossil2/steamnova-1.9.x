...
class statbuilder
{
	...
	private function GetUsersInfosFromDB()
	{
		...
		$Return['Alliance']	= $db->select('SELECT SQL_BIG_RESULT DISTINCT a.ally_owner, a.id, a.ally_universe, s.tech_rank AS old_tech_rank, s.build_rank AS old_build_rank, s.defs_rank AS old_defs_rank, s.fleet_rank AS old_fleet_rank, s.total_rank AS old_total_rank FROM %%ALLIANCE%% as a LEFT JOIN %%ALLIANCE_POINTS%% as s ON s.id_ally = a.id GROUP BY a.id;');
	...
	final public function MakeStats()
	{
		...
		{
			$AllySQL = $saveAllySQL = "INSERT INTO %%ALLIANCE_POINTS%% ( id_ally,id_owner, universe, tech_old_rank, tech_points, tech_count, build_old_rank, build_points, build_count, defs_old_rank, defs_points, defs_count, fleet_old_rank, fleet_points, fleet_count, total_old_rank, total_points, total_count) VALUES ";
