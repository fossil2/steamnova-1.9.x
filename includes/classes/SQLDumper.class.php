...
class SQLDumper
{
	...
	private function nativeDumpToFile($dbTables, $filePath)
	{
		...
		$sqlDump = shell_exec(
			"mysqldump --host=" . escapeshellarg($database['host']) .
			" --port=" . ((int) $database['port']) .
			" --user=" . escapeshellarg($database['user']) . " " . $passwordArgument .
			" --no-create-db --order-by-primary --add-drop-table --comments --complete-insert --hex-blob " .
			escapeshellarg($database['databasename']) . " " . implode(' ', $dbTables) .
			" 2>&1 1> " . $filePath
		);

		// FIXED: strlen(null) → safe check
...
/*!40101 SET character_set_client = utf8 */;\n\n");
			...
			if($createTableSql === false) {
				throw new Exception("Error after executing SHOW CREATE TABLE ".$dbTable."! Can't find key 'Create Table'. Data:\n".print_r($createTable, true));
			}
...
-- Dump completed on ".date("Y-d-m H:i:s"));
	...
	public function restoreDatabase($filePath)
	{
		...
		if($this->canNative('mysql'))
		{
			...
			$sqlDump = shell_exec(
				"mysql --host='" . escapeshellarg($database['host']) .
				"' --port=" . ((int) $database['port']) .
				" --user='" . escapeshellarg($database['user']) .
				"' --password='" . escapeshellarg($database['userpw']) .
				"' '" . escapeshellarg($database['databasename']) .
				"' < " . escapeshellarg($filePath) . " 2>&1 1> /dev/null"
			);
